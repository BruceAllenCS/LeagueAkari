...
function createHorseRatingLcuGame(queueId = 420, puuid = 'p1') {
  const puuids = [puuid, 'teammate-2', 'teammate-3', 'teammate-4', 'teammate-5']
  const emptyStats = {
    kills: 0,
    deaths: 0,
    assists: 0,
  ...
  return {
    ...
    data: {
      queueId,
      gameCreation: Date.now() - 60 * 60 * 1000,
      gameDuration: 30 * 60,
      participantIdentities: puuids.map((participantPuuid, index) => ({
        participantId: index + 1,
        player: { puuid: participantPuuid }
      })),
...
describe('in-game-send presets', () => {
  ...
  it('builds legacy seven-tier rating lines from current-queue matches', () => {
    const mainContext = createMainContext()
    mainContext.ongoingGame.state.matchHistory = {
      p1: {
        data: [createHorseRatingLcuGame(420)]
      }
    }
    ...
    expect(lines).toEqual([
      '通天代 无极剑圣 评分: 316，近1场KDA 30.0 胜率 100%',
      '盲僧 当前模式近期无有效对局'
    ])
  })

  it('does not use a match from another queue for rating output', () => {
    const mainContext = createMainContext()
    mainContext.ongoingGame.state.matchHistory = {
      p1: {
        data: [createHorseRatingLcuGame(440)]
      }
    }
    ...
    expect(lines).toEqual(['无极剑圣 当前模式近期无有效对局'])
  })

  it('builds horse rating lines in English', async () => {
    ...
    expect(lines).toEqual([
      'Divine Carry Master Yi Rating: 316, last 1 match KDA 30.0 Win Rate 100%'
    ])
  })

  it('keeps the configurable player-name display strategy', () => {
    const mainContext = createMainContext()
    mainContext.ongoingGame.state.matchHistory = {
      p1: {
        data: [createHorseRatingLcuGame(420)]
    ...
    expect(lines).toEqual(['通天代 无极剑圣（RIP董事长#81406） 评分: 316，近1场KDA 30.0 胜率 100%'])
  })
